...
function CustomerChargesTab({ shipmentId, navigate }) {
  ...
  const updateLine = (key, field, value) =>
    updateLineFields(key, { [field]: value });
  ...
  const toggleRegion = (key, currentType) => {
    updateLine(
      key,
      "charge_type",
      currentType === "DISBURSEMENT" ? "SERVICE" : "DISBURSEMENT",
    );
  };
  ...
  const onPickCatalog = (key, val, row) => {
    ...
    if (item) {
      if (!row.don_vi_tinh && item.don_vi_tinh)
        patch.don_vi_tinh = item.don_vi_tinh;
      if (!row.don_gia && item.don_gia_mac_dinh != null)
        patch.don_gia = item.don_gia_mac_dinh;
      ...
      if (row.vat_percent == null && item.vat_percent_mac_dinh != null)
        patch.vat_percent = item.vat_percent_mac_dinh;
    }
    ...
    try {
      ...
      message.error(
        e?.response?.data?.error ||
          "Không thêm được vào danh mục (có thể tên đã tồn tại)",
      );
    } finally {
  ...
  const localTotals = useMemo(() => {
    return lines.reduce(
      (acc, l) => {
        ...
        return {
          subtotal: acc.subtotal + thanhTien,
          vat: acc.vat + vatAmount,
          grand_total: acc.grand_total + thanhTien + vatAmount,
        };
      },
    ...
    },
    {
      ...
      render: (v, r) => (
        <InputNumber
          ...
          value={v}
          min={0}
          onChange={(val) => updateLine(r.key, "so_luong", val)}
        />
      ),
    },
    {
      ...
      render: (_, r) => (
        <Space size={4}>
          <Tooltip
            title={
              kind === "service"
                ? "Chuyển dòng này sang vùng Chi hộ"
                : "Chuyển dòng này sang vùng Cước dịch vụ"
            }
          >
            <Button
              size="small"
              icon={<SwapOutlined />}
              onClick={() => toggleRegion(r.key, r.charge_type)}
            />
          </Tooltip>
          <Button
            size="small"
              ...
              <div>
                ...
                >
                  ...
                  <Button
                    size="small"
                    type="primary"
                    loading={addingCatalog}
                    onClick={() => quickAddCatalog(r.key)}
                  >
                    Thêm
                  </Button>
                </div>
  ...
  return (
    <div>
      ...
      >
        Đây là khoản SẼ THU KHÁCH — độc lập hoàn toàn với Cost bên tab "Chi phí"
        (giá bán không nhất thiết bằng chi phí thực tế). Lần đầu mở tab này, dữ
        liệu được copy từ Cost sang; sau đó sửa ở đây không ảnh hưởng Cost, và
        ngược lại.{" "}
        <b>Đây là nguồn dữ liệu DUY NHẤT cho doanh thu của lô hàng</b> — không
        còn ô "Cước dịch vụ (Doanh thu)" nhập tay ở phần Thông tin chung nữa. 2
        vùng dưới đây tự mang đúng loại của mình (Cước dịch vụ / Chi hộ) — không
        cần tự chọn Charge Type nữa; dùng nút <SwapOutlined /> ở cuối dòng nếu
        lỡ thêm nhầm vùng.
      </Typography.Text>
      ...
      >
        <span>
          Subtotal: <b>{formatMoney(localTotals.subtotal)}</b>
        </span>
        <span>
          VAT: <b>{formatMoney(localTotals.vat)}</b>
        </span>
        <span>
          Grand Total: <b>{formatMoney(localTotals.grand_total)}</b>
        </span>
      </div>
      <div
        style={{
      ...
      >
        <Button
          ...
          onClick={() => navigate(`/debit-notes/new?shipment_id=${shipmentId}`)}
        >
...
export default function ShipmentForm() {
  ...
  const [charges, setCharges] = useState(() =>
    isEdit ? [] : [makeEmptyCharge(), makeEmptyCharge()],
  );
  ...
  const quickAddSupplier = async (rowKey) => {
    ...
    try {
      ...
      message.error(
        e?.response?.data?.error ||
          "Không thêm được NCC (có thể tên đã tồn tại)",
      );
    } finally {
  ...
  const tongChiPhi = useMemo(
    () => charges.reduce((a, c) => a + (Number(c.so_tien) || 0), 0),
    [charges],
  );
  const tongChiHo = useMemo(
    () =>
      charges.reduce(
        (a, c) => a + (c.la_chi_ho ? Number(c.so_tien) || 0 : 0),
        0,
      ),
    [charges],
  );
  const doanhThuDichVuDuKien = isEdit
    ? savedDoanhThuDichVu
    : tongChiPhi - tongChiHo;
  ...
  const columns = [
    ...
    },
    {
      ...
      render: (v, row) => (
        <InputNumber
          {...moneyProps}
          value={v}
          onChange={(val) => updateCharge(row.key, { so_tien: val || 0 })}
        />
      ),
    },
    {
      ...
      render: (v, row) => (
        <Checkbox
          checked={v}
          onChange={(e) =>
            updateCharge(row.key, { da_thanh_toan: e.target.checked })
          }
        />
      ),
    },
    {
      ...
      render: (v, row) => (
        <Checkbox
          checked={v}
          onChange={(e) =>
            updateCharge(row.key, { la_chi_ho: e.target.checked })
          }
        />
      ),
    },
    ...
    },
    {
      ...
      render: (v, row) => (
        <Input
          value={v}
          onChange={(e) => updateCharge(row.key, { ghi_chu: e.target.value })}
        />
      ),
    },
    {
      ...
      render: (_, row) => (
        <Popconfirm
          title="Xoá dòng chi phí này?"
          onConfirm={() => removeCharge(row.key)}
        >
  ...
  const linkedColumns = (isThu) => [
    ...
    { title: "Nội dung", dataIndex: "ghi_chu" },
    {
      ...
      width: 140,
      render: (v) => (
        <span style={{ color: isThu ? "#389e0d" : "#cf1322" }}>
          {formatMoney(v)}
        </span>
      ),
    },
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const payload = {
        ...values,
        ngay_ct: ngayCtStr,
        ngay_to_khai: values.ngay_to_khai
          ? values.ngay_to_khai.format(DATE_FMT)
          : null,
      ...
      <Form form={form} layout="vertical" disabled={loading}>
        ...
        >
          ...
          >
            <Form.Item
              ...
              initialValue={dayjs()}
              rules={[
                { required: true, message: "Vui lòng chọn ngày chứng từ" },
              ]}
            ...
            <Form.Item
              label="PO"
              name="po"
              tooltip="Dùng để hiển thị trên Debit Note (nếu có)"
            >
              <Input />
            </Form.Item>
        ...
        <div style={{ background: "#fff", padding: 24, borderRadius: 8 }}>
            ...
            items={[
              {
                ...
                children: (
                  <>
                    ...
                    >
                      Lưu ý: Lưu lô hàng sẽ tự tạo (hoặc cập nhật lại) phiếu
                      thu/phiếu chi thật cho những khoản đã tick "Đã thu?"
                      (cước) / "Đã thanh toán?" (từng dòng chi phí) — nội dung
                      tự sinh theo mẫu "TK {"{"}số tờ khai{"}"} - Thu cước/Chi{" "}
                      {"{...}"} - {"{"}mã lô{"}"}". Bỏ tick rồi Lưu sẽ tự xoá
                      phiếu tương ứng. Vẫn có thể tạo phiếu tay khác (không gắn
                      theo cơ chế này) ở menu "Phiếu thu / chi" hoặc "Công nợ
                      KH/NCC".
                    </Typography.Text>
                  </>
                ),
              },
              {
                ...
                children: isEdit ? (
                  <CustomerChargesTab
                    key={chargesRefreshKey}
                    shipmentId={id}
                    navigate={navigate}
                  />
                ) : (
                  <Typography.Text type="secondary">
                    Lưu lô hàng trước để dùng tab này.
                  </Typography.Text>
                ),
              },
            ]}
          ...
          >
            ...
            <span
              style={{ color: loiNhuanDuKien >= 0 ? "#389e0d" : "#cf1322" }}
            >
